...
var staticMessages = {
   tr: {
      errDefault: "Bir hata oldu. Lütfen daha sonra tekrar deneyin.",
      msgKvkk: "Lütfen Kişisel Verilerin Korunması ve Aydınlatma Metni'ni kabul ettiğinizi onaylayın."
   },
   en: {
      errDefault: "Something went wrong. Please try again later.",
      msgKvkk: "Please confirm you have read and accept the Personal Data Protection and Disclosure Text."
   },
   ru: {
      errDefault: "Что-то пошло не так. Пожалуйста, повторите попытку позже.",
      msgKvkk: "Пожалуйста, подтвердите, что вы прочитали и принимаете Текст о защите и раскрытии персональных данных."
   }
}
...
$(document).ready(function () {
   ...
   $('.swiper-container').each(function () {
      ...
      if (slideID === "projects-slider") {
         sectionProjects = $('#projects-slider-holder');
         sectionProjects.find('.project-image:first-child').addClass('active');
         data.on = {
            slideChangeTransitionEnd: function () {
               sectionProjects.find('.project-image.active').removeClass('active');
               sectionProjects.find('.project-image:eq(' + this.activeIndex + ')').addClass('active');
            }
         };
      }
   ...
   $('#contact-form').submit(function () {
      ...
      if (true /* elem.find('#input-terms')[0].checked */ ) {
         $.ajax({
            type: 'POST',
            ...
            success: function (result) {
               stage.html('<p class="message-holder">' + result.message + '</p>');
            },
            error: function (error) {
               console.log(error.responseText);
               stage.html('<p class="message-holder">' + staticMessages[currentLang].errDefault + '</p>');
            },
            complete: function () {
               setTimeout(function () {
                  stage.fadeOut().empty();
         ...
         stage.html('<p class="message-holder">' + staticMessages[currentLang].msgKvkk + '</p>');
         setTimeout(function () {
            stage.fadeOut().empty();
         }, 2400);
      }
   ...
   var cookiePopup = $('#cookie-consent-popup');

   if (cookiePopup.length > 0) {

      var checkboxes = cookiePopup.find('input[type="checkbox"]');
      var psw_uuid = getCookieValuePSW('psw_uuid');
      ...
      function getCookieValuePSW(cookieName) {
      ...
      if (!psw_uuid) {
         ...
         $.ajax({
            type: 'POST',
            url: '/api/getclientconsent',
            data: "",
            success: function (result) {

               var userLang = navigator.language || navigator.userLanguage

               setCookie("psw_uuid", JSON.parse(result.message), 120);
               psw_uuid = getCookieValuePSW('psw_uuid');

               var labelText = "Onay ID: " + psw_uuid;

               if (userLang.startsWith("en")) {
                  labelText = "Confirmation ID: " + psw_uuid;
               } else if (userLang.startsWith("ru")) {
                  labelText = "Подтверждение ID: " + psw_uuid;
               }

               $("#successIDLabel").html(labelText);
      ...
      function saveCookieSettings(timeout = 1) {


         var analyticsCheckbox = cookiePopup.find('input[name="analytics"]');
         var functionalityCheckbox = cookiePopup.find('input[name="functionality"]');
         var targetingCheckbox = cookiePopup.find('input[name="targeting"]');

         var cookieValue = 'set:';
         cookieValue += analyticsCheckbox.is(':checked') ? 'analytics,' : '';
         cookieValue += functionalityCheckbox.is(':checked') ? 'functionality,' : '';
         cookieValue += targetingCheckbox.is(':checked') ? 'targeting' : '';

         document.cookie = 'cookiePreferences=' + cookieValue + '; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/';

         var cookieOptions = {
            "analytics": analyticsCheckbox.is(':checked') ? true : false,
            "functionality": functionalityCheckbox.is(':checked') ? true : false,
            "targeting": targetingCheckbox.is(':checked') ? true : false,
            "psw_uuid": psw_uuid,
         }

         $.ajax({
            type: 'POST',
            url: '/api/setcookiepreferences',
            data: {
      ...
      cookiePopup.find('#cookie-accept-all').click(function () {
         checkboxes.each(function () {
            $(this).prop('checked', true);
         });

         saveCookieSettings(400);
      });

      cookiePopup.find('#cookie-reject-all').click(function () {
         checkboxes.each(function () {
      ...
      cookiePopup.find('#cookie-save-settings').click(function () {
         saveCookieSettings(10);
      });

   }
});;
var industrialMessages = {
   tr: {
      directions: 'Yol Tarifi',
      chooseCity: 'Lütfen Şehir Seçin',
      chooseProvince: 'Lütfen İlçe Seçin',
      chooseDealer: 'Lütfen Satış Noktası Seçin',
   },
   en: {
      directions: 'Directions',
      chooseCity: 'Please Choose City',
      chooseProvince: 'Please Choose District',
      chooseDealer: 'Please Choose Sales Point',
   },
   ru: {
      directions: 'Направления',
      chooseCity: 'Пожалуйста, выберите регион',
      chooseProvince: 'Пожалуйста, выберите город',
      chooseDealer: 'Пожалуйста, выберите пункт продажи',
   }
};
$(document).ready(function () {
   var stage = $('#stage');
   ...
   if (mapHolder.length > 0) {
      var map;
      var chooseSector = $('#choose-sector');
      var chooseCity = $('#choose-city');
      var chooseProvince = $('#choose-province');
      var chooseDealer = $('#choose-dealer');
      var topCities = ["İstanbul", "Ankara", "İzmir"];
      var foundDealers;
      ...
      function focusDealer(id) {
         ...
         $.each(dealers, function (index, dealer) {
            if (!found && dealer.id == id) {
               map.setCenter(new google.maps.LatLng(dealer.lat, dealer.lng));
               if (focusedDealer) {
                  focusedDealer.infoWindow.close();
               }
               focusedDealer = dealer;
               focusedDealer.infoWindow.open(map, focusedDealer.marker);
               found = true;
            }
         });
      };
      ...
      window.initMap = function () {
         map = new google.maps.Map(mapHolder[0], {
            center: new google.maps.LatLng(dealers[0].lat, dealers[0].lng),
            zoom: 14
         });
         ...
         $.each(dealers, function (index, item) {
            item.infoWindow = new google.maps.InfoWindow({
               content: '<div class="info-window"><h6>' + item.title + '</h6>' + item.address + '<br>' + item.tel + '<br><a href="https://www.google.com/maps/dir/?api=1&destination=' + item.lat + ',' + item.lng + '" target="_blank" class="btn">' + industrialMessages[currentLang].directions + ' <i class="fa fa-external-link"></i></a></div>'
            });

            item.marker = new google.maps.Marker({
               position: new google.maps.LatLng(item.lat, item.lng),
               icon: '/Content/gfx/icon-map-marker.png',
               map: map,
               index: index
            });
            ...
            item.marker.addListener('click', function () {
               ...
               focusedDealer = item;
               focusedDealer.infoWindow.open(map, item.marker);
            });
         });

         chooseSector.change(function () {
            sectorID = parseInt(this.value);
            ...
            $.each(dealers, function (index, item) {
               if (item.sectors.indexOf(sectorID) > -1) {
                  foundDealers.push(item);
            ...
            chooseCity.html('<option value="">' + industrialMessages[currentLang].chooseCity + '</option>').removeClass('disabled').prop('disabled', false);
            chooseProvince.html('<option value="">' + industrialMessages[currentLang].chooseProvince + '</option>').addClass('disabled').prop('disabled', true);
            chooseDealer.html('<option value="">' + industrialMessages[currentLang].chooseDealer + '</option>').addClass('disabled').prop('disabled', true);

            var cities = [];

            $.each(foundDealers, function (index, item) {
            ...
            $.each(topCities, function (index, city) {
               if (cities.indexOf(city) > -1) {
                  chooseCity.append('<option value="' + city + '">' + city + '</option>');
         ...
         chooseCity.change(function () {
            ...
            foundDealers = dealers.filter(function (el) {
               return el.city == city && el.sectors.indexOf(sectorID) > -1;
            });

            chooseProvince.html('<option value="">' + industrialMessages[currentLang].chooseProvince + '</option>').removeClass('disabled').prop('disabled', false);
            chooseDealer.html('<option value="">' + industrialMessages[currentLang].chooseDealer + '</option>').addClass('disabled').prop('disabled', true);
            ...
            $.each(provinces, function (index, province) {
               chooseProvince.append('<option value="' + province + '">' + province + '</option>');
            });

            /*

            chooseProvince.val(provinces[0]);
            chooseProvince.trigger('change'); */
         });

         chooseProvince.change(function () {
            var province = this.value;

            var provinceDealers = foundDealers.filter(function (el) {
               return el.province == province;
            });

            chooseDealer.html('<option value="">' + industrialMessages[currentLang].chooseDealer + '</option>').removeClass('disabled').prop('disabled', false);

            $.each(provinceDealers, function (index, dealer) {
               chooseDealer.append('<option value="' + dealer.id + '">' + dealer.title + '</option>');
            });

            /* 
            chooseDealer.value = provinceDealers[0].id;
            focusDealer(chooseDealer.value); */
         });

         chooseDealer.change(function () {
            focusDealer(this.value);
         });
      };
      $('body').append('<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA9xqf3uiJBgQFPiyaNsjqwUHGGXFZMoDs&callback=initMap"></script>');
   }

   $('.pdf-opener').click(function () {
      ...
      if (file !== "") {
         ...
         if ($(window).width() < 768) {
            ...
            stage.html('<button id="stage-close" class="btn"></button><div id="pdf-holder" class="container"><embed src="' + $(this).data('file') + '" type="application/pdf"></div>');
